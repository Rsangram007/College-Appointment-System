...
afterAll(async () => {
  ...
  // Debug: Check for any lingering connections
  console.log("Connections:", mongoose.connection.readyState); // 0 means disconnected
});
...
describe("Appointment System", () => {
  it("should allow a student to book and cancel an appointment", async () => {
    ...
    // Fetch professor ID dynamically
    const professor = await mongoose.connection
      .collection("users")
      .findOne({ email: "professor1@example.com" });
    const professorId = professor._id;

    // Book an appointment
    const booking = await request(app)
      .post(`/api/student/book/${professorId}`)
